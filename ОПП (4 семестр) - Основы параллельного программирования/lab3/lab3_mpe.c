...
void unite_matrix(MPI_Comm comm2d, int* C, int* C_, int rank, int size, int n1, int n2, int n3, int p1, int p2, int id_begin_phase_4, int id_end_phase_4)
{
	...
	//printf("check 9; %d\n", rank);
	//MPI_Barrier(comm2d);
	MPE_Log_event(id_begin_phase_4, rank, (char*)0);
	MPI_Gatherv(C_, (n1 / p1) * (n3 / p2), MPI_INT, C, recvcounts, displs, C_vector_, 0, comm2d);
	MPE_Log_event(id_end_phase_4, rank, (char*)0);
	//MPI_Barrier(comm2d);
	printf("check 10; %d\n", rank);
...
void send_chunks(MPI_Comm comm2d, int size, int rank, int* A, int* B, int* C, int* A_, int* B_, int* C_, int n1, int n2, int n3, int p1, int p2, int id_begin_phase_5, int id_end_phase_5)
{
	int color_A = rank / p2;
	int color_B = rank % p2;
	int sizeA;
	...
	MPE_Log_event(id_begin_phase_5, rank, (char*)0);
	if (rank == rankA * sizeB)
	{
	...
	MPE_Log_event(id_end_phase_5, rank, (char*)0);
...
void go(int n1, int n2, int n3, int p1, int p2)
{
	...
	MPE_Describe_state(id_begin_phase_1, id_end_phase_1, "Create matrix", "red");
	MPE_Describe_state(id_begin_phase_2, id_end_phase_2, "Multiply", "green");
	MPE_Describe_state(id_begin_phase_3, id_end_phase_3, "Checking", "cyan");
	MPE_Describe_state(id_begin_phase_4, id_end_phase_4, "Unite", "yellow");
	MPE_Describe_state(id_begin_phase_5, id_end_phase_5, "Partition", "blue");
	...
	MPE_Log_event(id_begin_phase_1, rank, (char*)0);
	memory_matrix(&A, &B, &C, &A_, &B_, &C_, rank, n1, n2, n3, p1, p2); //выделяем память и создаём матрицы
	MPE_Log_event(id_end_phase_1, rank, (char*)0);

	//printf("check 4\n");

	send_chunks(comm2d, size, rank, A, B, C, A_, B_, C_, n1, n2, n3, p1, p2, id_begin_phase_5, id_end_phase_5); //рассылка кусочков всем процессам
	
	//printf("check 5\n");

	MPE_Log_event(id_begin_phase_2, rank, (char*)0);
	multiply(C_, A_, B_, n1 / p1, n2, n3 / p2); //перемножение матриц
	MPE_Log_event(id_end_phase_2, rank, (char*)0);

	//printf("check 6; %d\n", rank);

	unite_matrix(comm2d, C, C_, rank, size, n1, n2, n3, p1, p2, id_begin_phase_4, id_end_phase_4); //сбор матриц в единое целлое
	//printf("\nC: \n");
	//print_matrix(C, n1, n3, rank);

	//printf("check 7; %d\n", rank);

	MPE_Log_event(id_begin_phase_3, rank, (char*)0);
	check_result(comm2d, C, C_, rank, n1, n3, p1, p2); //проверка результатов
	MPE_Log_event(id_end_phase_3, rank, (char*)0);
