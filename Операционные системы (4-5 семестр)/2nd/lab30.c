...
void* receiver(void* args)
{
    while (1)
    {
        ...
        do_recv = 1; //делаем recv, поэтому ставим 1 на верм€
...
void urlViewer(char* url)
{
    ...
    while (1)
    {
        ...
        result = select(socket_fd_ + 1, &reads, &writes, NULL, NULL); //Select скажет на каких сетах произошло событие
        ...
        {
            //if (top != SIZE_BUF && FD_ISSET(socket_fd, &reads)) //” нас есть место в буфере и произошло событие на сокете
            //{
