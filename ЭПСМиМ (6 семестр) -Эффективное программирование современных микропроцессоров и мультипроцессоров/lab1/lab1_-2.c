...
double steps(double* F_new, double* F_old, double* ro, double h_x, double h_y, int Nx, int Ny)
{
    ...
    for (int i = 1; i <= Ny - 2; i++) // < ?
    {
        ...
        {
            F_new[i * Nx + j] = 0.2 * (1.0 / (1.0 / (h_x * h_x) + 1.0 / (h_y * h_y))) * (
                (0.5 * (5.0 / (h_x * h_x) - 1.0 / (h_y * h_y))) * (F_old[i * Nx + j - 1] + F_old[i * Nx + j + 1])
                + (0.5 * (5.0 / (h_y * h_y) - 1.0 / (h_x * h_x))) * (F_old[i * Nx + j + Nx] + F_old[i * Nx + j - Nx])
                + 0.25 * (1.0 / (h_x * h_x) + 1.0 / (h_y * h_y)) * (F_old[i * Nx + j - Nx - 1] + F_old[i * Nx + j - Nx + 1] + F_old[i * Nx + j + Nx - 1] + F_old[i * Nx + j + Nx + 1])
                + 2.0 * ro[i * Nx + j] + 0.25 * (ro[i * Nx + j - Nx] + ro[i * Nx + j + Nx] + ro[i * Nx + j - 1] + ro[i * Nx + j + 1]) //old sdvig
            );
