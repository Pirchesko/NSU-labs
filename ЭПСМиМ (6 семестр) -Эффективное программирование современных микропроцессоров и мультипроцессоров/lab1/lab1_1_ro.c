...
double steps(double* F_new, double* F_old, double* ro, double X_h_x2_h_y2, double Y_h_x2_h_y2, double koef_1_5, double koef_1_4, int Nx, int Ny)
{
    ...
    for (int i = 1; i <= Ny - 2; i++) // < ?
    {
        ...
        for (int j = 1; j <= Nx - 2; j++) // < ?
        {
            ...
            i_jp1 = index + 1;
            i_jm1 = index - 1;
            F_new[index] = koef_1_5 * (
                X_h_x2_h_y2 * (F_old[i_jm1] + F_old[i_jp1])
                + Y_h_x2_h_y2 * (F_old[ip1_j] + F_old[im1_j])
                + koef_1_4 * (F_old[im1_j - 1] + F_old[im1_j + 1] + F_old[ip1_j - 1] + F_old[ip1_j + 1])
                + ro[index] //old sdvig
            );
...
int main()
{
    ...
    ro[0] = (double*)calloc(Nx * Ny, sizeof(double));
    ro[1] = (double*)calloc(Nx * Ny, sizeof(double));
    ...
    for (int i = 0; i < Nt; i++)
    {
        delta_new = steps(F[(id + 1) % 2], F[id % 2], ro[1], X_h_x2_h_y2, Y_h_x2_h_y2, koef_1_5, koef_1_4, Nx, Ny);
